...
class EjemploFuncionCostosa1a {
  ...
  public static void main( String args[] ) {
    ...
    MiHebraCiclica[] miHebras = new MiHebraCiclica[numHebras];
    for (int i = 0; i < miHebras.length; i++) {
      miHebras[i] = new MiHebraCiclica(i, numHebras, n, vectorX, vectorY);
      miHebras[i].start();
      try {
        miHebras[i].join();
    ...
    inicializaVectorY(vectorY);
    t1 = System.nanoTime();
    MiHebraBloque[] miHebraBloque = new MiHebraBloque[numHebras];
    for (int i = 0; i < miHebras.length; i++) {
      miHebraBloque[i] = new MiHebraBloque(i, numHebras, n, vectorX, vectorY);
      miHebraBloque[i].start();
      try {
        miHebraBloque[i].join();
      } catch (InterruptedException e) {
        e.printStackTrace();
      }
    }
    ...
    System.out.println( "Incremento paralela bloques:                 " + ((tb-ts)/ts)*100);
    //// imprimeResultado( vectorX, vectorY );
    // Comprueba el resultado.
    sumaX = sumaVector( vectorX );
    sumaY = sumaVector( vectorY );
